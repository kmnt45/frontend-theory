...
2. Асинхронные операции, которые требуют внешней среды (таймеры, сетевые запросы, события), инициируются через Web API/Node API; асинхронные операции, которые не требуют внешней среды, инициируются внутри JS-движка, но выполняются через очередь микротасок.
