...
`React Fiber` — это механизм рендеринга в React, появившийся начиная с версии 16.
Он был создан для того, чтобы сделать процесс обновления интерфейса асинхронным, управляемым и приоритетным.
...
2. `Commit` — React применяет подготовленные изменения к DOM.
...
Все связи начинаются с Host Root — корневого Fiber.
...
**Приоритетность задач и прерывание работы**

React может приостанавливать и возобновлять процесс рендеринга.
Каждой задаче присваивается приоритет планировщиком (Scheduler):
...
Другие способы обработки ошибок в React:
...
## 22. Что такое строгий режим в React? Его преимущества?
...
## 32. Как отрендерить HTML код в React-компоненте?
