...
2. Сервер отвечает заголовками, определяющими разрешения:
   ...
   - `Access-Control-Expose-Headers` — заголовки ответа, доступные для чтения из JavaScript.

3. Если preflight-ответ удовлетворяет браузеру, выполняется основной запрос. Если нет — запрос блокируется.
...
3. `Cookies`
   ...
   - Поддерживают флаги HttpOnly (Запрещает доступ к куки из JS (через document.cookie)), Secure (Разрешает отправку только по HTTPS), SameSite (Контролирует поведение при кросс-доменных запросах) для безопасности.
