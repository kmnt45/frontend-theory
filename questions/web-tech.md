...
1. **Браузер сначала ищет ресурс в кэше:**
...
2. **Если кэша нет, браузер выполняет DNS-запрос, чтобы узнать IP-адрес домена. Проверяются:**
...
3. **Между клиентом и сервером устанавливается TCP-соединение (трёхэтапное рукопожатие — 3-way handshake).**
...
4. **Если HTTPS, устанавливается защищённое соединение (TLS Handshake)**
...
5. **[Браузер отправляет HTTP-запрос (HTTP-request).](#1-http-запросыответы)**

6. **[Ответ сервера (HTTP-response).](#1-http-запросыответы)**

7. **[Парсинг и рендеринг страницы.](#3-стадии-работы-браузера)**
...
1. **Parsing (Парсинг)**
    ...
    - Одновременно подгружаются остальные ресурсы (скрипты, изображения и шрифты).
