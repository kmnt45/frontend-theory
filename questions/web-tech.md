...
2. Сервер в ответе указывает:
   ...
   - `Access-Control-Allow-Origin` — указывает, какой источник (origin) может получать доступ к ресурсу. Может быть как конкретный домен, так и * для разрешения любому источнику.

   - `Access-Control-Allow-Methods` — список HTTP-методов (например, GET, POST, DELETE), которые разрешены для запроса.

   - `Access-Control-Allow-Headers` — список заголовков, которые могут быть отправлены с запросом, например, Authorization или пользовательские заголовки.

   - `Access-Control-Allow-Credentials` — булевое значение (true), которое указывает, разрешается ли браузеру отправлять учетные данные (файлы cookie, авторизационные заголовки) с запросом.

   - `Access-Control-Max-Age` — время (в секундах), в течение которого результаты предварительного запроса (preflight request) могут кэшироваться.

    Дополнительные заголовки:

    - `Access-Control-Expose-Headers` — список заголовков, которые могут быть доступны клиентской стороне из ответа сервера.

    - `Access-Control-Request-Headers` — заголовок, который браузер отправляет в предварительном запросе, чтобы сообщить серверу, какие заголовки будут использоваться в основном запросе.

3. Если ответ удовлетворяет условиям браузера, отправляется основной запрос.
