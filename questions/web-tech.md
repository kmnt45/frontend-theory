...
- Проверяет подлинность сервера с помощью TLS-сертификата (часто называемого "SSL-сертификатом").
...
| Скорость   | Средняя (задержки из-за TCP handshake) | Быстрее (меньше задержек, мультиплексирование) | Ещё быстрее (нет потерь на TCP, 0-RTT handshake) |
